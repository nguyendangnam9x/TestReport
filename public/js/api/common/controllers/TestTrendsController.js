angular.module('ExtentX').controller('TestTrendsController', ['$scope', 'Aggregates', 'DateTime', 'LineChartSettings', 'DataPointFormat', 'CustomDataChart',
    function ($scope, Aggregates, DateTime, LineChartSettings, DataPointFormat, CustomDataChart) {
        Aggregates.then(function (response) {
            getMyAggregatesTest(response);
        });

        $scope.$on('MyAggregatesTest', function (event, message) {
            getMyAggregatesTest(message.responseData);
        });

        function getMyAggregatesTest(response) {
            if (response != "") {
