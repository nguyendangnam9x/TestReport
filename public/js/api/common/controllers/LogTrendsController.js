angular.module('ExtentX').controller('LogTrendsController', ['$scope', 'Aggregates', 'DateTime', 'LineChartSettings', 'DataPointFormat', 'CustomDataChart',
    function ($scope, Aggregates, DateTime, LineChartSettings, DataPointFormat, CustomDataChart) {
        Aggregates.then(function (response) {
            getAggregatesLog(response);
        });

        $scope.$on('MyAggregatesLog', function (event, message) {
            getAggregatesLog(message.responseData);
        });

        function getAggregatesLog(response) {
            if (response != "") {
